...
def batch_delete_files_view(request):
    ...
    POST 接收 ids 或 file_ids 列表，逐个校验权限后删除（含物理文件），返回成功删除数量。
    """
    if request.method == 'POST':
        file_ids = request.POST.getlist('ids[]') or request.POST.getlist('ids') or request.POST.getlist('file_ids')
        deleted_count = 0
        ...
        return json_success('file.batch_delete.success', data={'count': deleted_count})
