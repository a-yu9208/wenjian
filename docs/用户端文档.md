...
- **可见性选择**：
  ...
  - **私密**：仅自己可见，无需审核
- **自动处理**：上传后系统通过定时任务自动提取视频封面（缩放至640×360）和时长（通常2分钟内完成）
...
- **预览**：文本类文件（txt/md等）支持在线预览前5000字符
- **支持 AJAX**：请求头含 `X-Requested-With: XMLHttpRequest` 或 URL 参数 `?format=json` 时返回 JSON
