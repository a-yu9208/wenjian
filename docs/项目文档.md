...
| `views.py` | 注册(含60秒限流)/登录(含登录日志)/登出/个人中心(含用户视频和文件列表)/获取网站设置API/更新网站设置API/修改视频可见性/修改文件可见性 | core.message_service、core.review_service、core.models.LoginLog、videos.models、files.models |
...
| `views.py` | 文件列表(分类筛选+搜索+分页)、文件详情(JSON/HTML，session防刷)、文件下载(Nginx X-Accel-Redirect，私密权限校验)、上传(自动识别类型)、删除/批量删除(事务保护)、审核通过/驳回 | core.review_service、core.message_service |
...
| `views.py` | 首页视频列表(搜索+排序+分页)、视频详情(session防刷)、视频流播放(Nginx X-Accel-Redirect)、上传(标记待处理，由定时任务提取封面和时长)、删除/批量删除(事务保护)、审核通过/驳回 | core.review_service、core.message_service、videos.utils |
...
| `/files/<id>/` | files.views.file_detail_view | 文件详情 |
...
- 超时设置 30 分钟，支持大文件上传
- 视频封面/时长由定时任务 `python manage.py process_videos` 处理（建议 crontab 每2分钟）
