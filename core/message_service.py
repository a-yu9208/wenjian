...
class MessageService:
    ...
    MESSAGES = {
        ...
        'user.password.wrong_old': '原密码不正确',
        'user.password.mismatch': '两次输入的新密码不一致',
        'user.password.too_short': '新密码至少需要6位',
    ...
    def json_response(
        ...
        **kwargs
        ...
        if message_key:
            # 合并 data 和 kwargs，用于消息模板替换
            template_kwargs = {}
            if data:
                template_kwargs.update(data)
            template_kwargs.update(kwargs)
            response_data['message'] = MessageService.get_message(message_key, **template_kwargs)
        if data:
